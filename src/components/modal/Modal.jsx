...
export const ModalContext = ({ children, setIsModal, isModal, direction, title, Icon }) => {
        ...
        mount ? <ContainerModal onClick={setIsModal} active={!isModal}>
            <Modal active={isModal} onClick={e => e.stopPropagation()} >
                <ContainerCard direction={direction} borderBottom={` 1px solid ${ PLColor } `} border='none' width='auto'>
                    {Icon ? <BtnClose onClick={setIsModal} >X</BtnClose> : <BtnCloseCondition onClick={setIsModal} >X</BtnCloseCondition>}
                    <h3>{title}</h3>
                    {children}
                </ContainerCard>
            </Modal>
